...
public abstract class URMovingProjectile extends PersistentProjectileEntity {
    ...
    public void playSound(SoundEvent sound, float volume, float pitch) {
        if (!isSilent()) getWorld().playSound(getX(), getY(),getZ(), sound, getOwner() != null ? getOwner().getSoundCategory() : SoundCategory.NEUTRAL, volume, pitch,true);
    }
    ...
    public void spawnEffectParticles(int amount, int color) {
        ...
        for (int j = 0; j < amount; ++j) {
            getWorld().addParticle(EntityEffectParticleEffect.create(ParticleTypes.ENTITY_EFFECT, r, g, b), getParticleX(0.5), getRandomBodyY(), getParticleZ(0.5), r, g, b);
        }
    }
}